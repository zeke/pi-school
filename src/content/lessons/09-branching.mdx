...
agentInstructions: |
  ...
  4. Branch summaries: they're offered when you leave one branch for
     another, not just when you open or browse /tree — the trigger is
     switching branches, not the tree view itself. Pi offers to
     summarize the abandoned branch and attach it at the new position, so
     useful context from the branch you're leaving carries forward into
     the branch you're switching to. Also cover the /tree vs /fork vs
     /clone distinction: /tree stays in the same session file and shows
     the full tree; /fork starts a new session file from a previous user
     message via a selector; /clone duplicates the current active branch
