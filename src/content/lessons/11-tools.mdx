...
If Pi fails to load the extension after restarting, with an error mentioning `@modelcontextprotocol/sdk` (e.g. `Cannot find module '@modelcontextprotocol/sdk/types.js'`), a peer dependency is missing from the install. Fix it with:

```bash
npm install --prefix ~/.pi/agent/npm @modelcontextprotocol/sdk@^1.29.0
```
