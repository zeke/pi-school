...
| 12 | branching | Branching | New lesson, split out of Sessions because Pi's tree model deserves its own space. Opens with the "why bother" framing (compare approaches without losing either, rewind without starting over) before mechanics. ASCII tree diagram as the visual anchor. Covers `/tree` navigation and controls, the user-message-vs-non-user-message selection behavior (edit-and-resubmit vs. jump-and-continue — a real subtlety worth calling out explicitly), branch summaries when switching away from a path, and a `/fork` vs. `/clone` vs. `/tree` comparison table. Hands-on exercise: ask Pi to do a task one way, `/tree` back to before that response, ask it a different way, then `/tree` again to compare both branches side by side. No OpenCode equivalent — pure upgrade content. |
